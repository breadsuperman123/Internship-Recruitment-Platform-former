    ...
    actions: {
        ...
        async createWebSocket(nickname: string) {
            ...
            return new Promise((resolve, reject) => {
                ...
                // 建立WebSocket全双工通信链接
                this.webSocketInstance = new WebSocket('ws://127.0.0.1:8081/websocket/' + nickname)
        ...
        async updateFriends(data: any) {
            ...
            if (this.friendsListInfo.length > 0) {
                ...
                for (let i = 0; i < data.messages.length; i++) {
                    const findIndex = this.friendsListInfo.findIndex((object: any) => object.nickname === data.messages[i].nickname);
                    // 验证是否一致
                    // 不一致 新增好友
                    if (findIndex === -1) {
                        const friend = {
                ...
                for (let i = 0; i < this.friendsListInfo.length; i++) {
                    // 查找好友
                    // 移除或者修改在线状态
                    const findIndex1 = data.messages.findIndex((object: any) => object.nickname === this.friendsListInfo[i].nickname);
                    if (findIndex1 == -1) {
                        // 不存在代表已经离线
